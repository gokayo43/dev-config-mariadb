...
# How every step of this action starts its own program. Sourced, not run.
#
...
gate_pinned() {
  ...
  if [ -z "${PATH:-}" ]; then
    echo "::error::the path input is empty — the calling job passes the search path this gate's own tools are found on, read before the graded repo's code could prepend to it"
    return 1
  fi
}
