...
/**
 * A real server and a real migrator per case, because every property this gate
 ...
 * `servers.ts` is the server; `tree.ts` is the project on disk.
 *
 * Every case runs against both products, which is the only way the claim that
 * this gate serves both is worth anything: the dump it rests on is taken by the
 * image's own client, and the two products have no client name in common — so a
 * gate that had `mariadb-dump` written into it passes every case here against
 * MariaDB and fails all of them against MySQL. Two cases are the dialect itself
 * rather than the gate, and each says so where it stands.
 */
...
/**
 * An image with no dump client in it, taken from the one image this repo still
 * pins for itself — the wrapper's Redis service. What the case below needs of
 * it is only that it is not a database server, and reading it here rather than
 * writing a second digest down means the suite pulls what the workflow already
 * names.
 */
const [NO_CLIENT] = await serviceImages();
if (NO_CLIENT === undefined) {
  throw new Error(
    "the wrapper declares no service image, and this suite needs one with no dump client in it",
  );
}
...
for (const product of PRODUCTS) {
  describe(product.name, () => {
    ...
    test("a second replay that changes the schema is refused, naming the lines that differ", async () => {
      ...
      expect(verdict.log).toContain(product.secondCheck);
    ...
    test("an empty database-image is refused before any migration runs", async () => {
    ...
    /**
     * The image is asked what it ships rather than told, and this is the answer
     * that has to be an answer rather than a crash: an image with no dump client
     * in it at all. The most plausible wrong implementation runs the two replays
     * first and then dies inside `docker run` with a message about an executable,
     * having spent the whole gate to report a pin somebody wrote wrong — so this
     * asserts the database is untouched as much as it asserts the sentence.
     *
     * Redis because it is a pin this repo already carries — the wrapper's own
     * service — and because any image that is not a database server proves the
     * same thing.
     */
    test("an image that ships no dump client is refused before any migration runs", async () => {
      const root = await materialise({
        ...migratesFrom(JOURNALLED, "drizzle"),
        ...lineage("drizzle", CREATES_THING),
      });
      ...
      expect(verdict.problems).toHaveLength(1);
      expect(verdict.problems[0]).toContain("ships neither of the dump clients");
      expect(await names(url)).toEqual([]);
    }, 120_000);
